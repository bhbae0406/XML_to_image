...
void Parser::reDraw()
{
         ...
         textblock = textblock->next_sibling("textblock"))
   {
      ...
      else
      {
         ...
         {
            cout << "object width: " << getobjectwidth(textline) << '\n';

            if ((getobjectwidth(textline) > widththresh) && 
...
/*
//used for trackbar
void Parser::displayImage(int, void*)
{
...
void Parser::testFontThreshold(const char* xmlFile)
{
   initPicture();

   first_textBlock = readImage(xmlFile);
   ...
   createTrackbar("widthThresh", "Threshold Result", &widthThresh, 14, 
         displayImage);
